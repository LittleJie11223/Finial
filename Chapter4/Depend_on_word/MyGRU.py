...
# Clear 0 in the training matrix
...
num_features = len(da.dic)

def mymodel():
    model = keras.Sequential([
        layers.Embedding(input_dim=num_features, output_dim=length_output, input_length=dim),
        layers.GRU(length_output*3, return_sequences=True),
        layers.GRU(length_output*3, activation='sigmoid', return_sequences=False)
    ])
