...
x_train = x_train.reshape((-1,length_input,size_input,1))/len(da.dic)
x_test = x_test.reshape((-1,length_input,size_input,1))/len(da.dic)

y_train=pad_sequences(y_train,dtype=float, maxlen=length_output*3)/len(da.dic)
y_test=pad_sequences(y_test,dtype=float, maxlen=length_output*3)/len(da.dic)
print(y_train.shape)
...
# model fit
history = mymodel.fit(x_train, y_train,  epochs=5, validation_split=0.1)
