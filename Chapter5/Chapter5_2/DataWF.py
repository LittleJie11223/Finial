...
# Define function: Take out the ingredients and corresponding recipes
# from the JSON file and save them as two lists list_ingredients and list_steps
def get_FData():
    ...
    return list_steps,list_ingredients
# clear the data: delete the stop words

# Define function: unify the information in the materials, that is, delete all the brackets and the content in the brackets
def clear_ingredients(list_ingredients):
    list2 = []
...
# Define function: clear stop words
def clear_StopWords(list_steps):
...
# define the delete list function
def delete_list(list,list_ingredients,list_steps):
    for i in range(len(list) - 1, -1, -1):
        list_steps.pop(list[i])
        list_ingredients.pop(list[i])
    return list_ingredients,list_steps

# Manually check out the unqualified data for the final deletion
def final_clear(list,list_ingredients,list_steps):
    for li in list:
        list_ingredients,list_steps=delete_list(li,list_ingredients,list_steps)
    return list_ingredients,list_steps


# define the dictionary function about the dataset(Phrase word)
def mydictionaryP(list1,list2):
    tok = tt.preprocessing.text.Tokenizer()
    listW=[]
    for ingredients in list1:
        for ingredient in ingredients:
            listW.append(ingredient)
    for steps in list2:
        for step in steps:
...
# define the dictionary function about the dataset(Single word)
def mydictionaryS(list1,list2):
    tok = tt.preprocessing.text.Tokenizer()
    listW=[]
    for ingredients in list1:
        lists = []
    ...
    dic=tok.word_index
    return dic
# Use the dictionary to encode and decode(Phrase word)
def myencodeP(dic,str):
    encode = []
    for i in str:
        encode.append(dic[i])
    return encode

def mydecodeP(dic,str):
    decode = ''
    for i in str:
...
# Use the dictionary to encode and decode(Single word)
def myencodeS(dic,str):
    encode = []
    for i in str:
        for one in i:
            encode.append(dic[one])
    return encode

def mydecodeS(dic,str):
    decode = ''
    for i in str:
        for key, value in dic.items():
            if value == i:
...
# build the input matrix and output matrix(Phrase)
# Firstly, make sure the size of input and output(Phrase)
def find_inputsizeP(list_ingredients):
    row_input=0
    column_input=1
...
def find_outputsizeP(list_steps):
    row_output=0
    column_output=3
    for step in list_steps:
        for i in range(3):
...
# build the input matrix(Phrase word)
def build_inputP(list_ingredients,dic,row_input, column_input):
    list_train=[]
    for ingredient in list_ingredients:
...
def build_outputP(list_steps,dic):
    list_test=[]
    for step in list_steps:
        list=[]
        for i in range(3):
...
# build the input matrix and output matrix(Single)
# Firstly, make sure the size of input and output(Single)
def find_inputsizeS(list_ingredients):
    row_input=0
    column_input=1
    for ingredient in list_ingredients:
        sum=0
...
def find_outputsizeS(list_steps):
    row_output=0
    column_output=3
    for step in list_steps:
        for i in range(3):
            if row_output <= len(step[i]):
                row_output = len(step[i])
    return row_output, column_output

# Fianlly, build the output, input matrix
# build the input matrix(Single)
def build_inputS(list_ingredients,dic,row_input, column_input):
    list_train=[]
    for ingredient in list_ingredients:
        A = np.zeros([row_input, column_input])
        list3 = []
...
# build the output matrix
def build_outputS(list_steps,dic):
    list_test=[]
    for step in list_steps:
        list=[]
        for i in range(3):
...
# Get the processed phrase data and all required parameters
def getDataP():
    # Retrieve data from the json file:
    list_steps, list_ingredients = get_FData()
    # slove the data:
    # First delete all the marks in parentheses
    list_ingredients = clear_ingredients(list_ingredients)
    # Then delete the stop words in the recipe step
    list_steps = clear_StopWords(list_steps)
    # Then clean the data after manual review:
    list_ingredients, list_steps = final_clear(get_deleteList(), list_ingredients, list_steps)
    # Make a dictionary of words
    dic = mydictionaryP(list_ingredients, list_steps)
    # build the input matrix and output matrix
    # Firstly, make sure the size of input and output
    row_input, column_input = find_inputsizeP(list_ingredients)
    row_output, column_output = find_outputsizeP(list_steps)
    # build the input matrix
    list_train = build_inputP(list_ingredients, dic, row_input, column_input)
    # build the output matrix
    list_test = build_outputP(list_steps, dic)

    listin = [row_input, column_input]
    listout = [row_output, column_output]
    print('Run successfully')
    return list_train,list_test,listin,listout,dic
...
def getDataS():
    ...
    list_ingredients, list_steps=final_clear(get_deleteList(),list_ingredients,list_steps)
    #Make a dictionary of words
    dic = mydictionaryS(list_ingredients, list_steps)
    # build the input matrix and output matrix
    # Firstly, make sure the size of input and output
    row_input, column_input=find_inputsizeS(list_ingredients)
    row_output, column_output=find_outputsizeS(list_steps)
    # build the input matrix
    list_train=build_inputS(list_ingredients,dic,row_input, column_input)
    #build the output matrix
    list_test=build_outputS(list_steps,dic)
