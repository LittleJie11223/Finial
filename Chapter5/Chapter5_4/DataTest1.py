...
#     listW22=[]
#     for step in steps:
#         for one in jieba.lcut(step):
#             listW22.append(one)
#     listW2.append(listW22)
listW2=listW2+listW1
model = Word2Vec(listW2, size=4, window=5, min_count=1, workers=4)

dic1 = {}
for recipe in listW2:
    for word in recipe:
...
def myencode1(str):
    ...
    for words in str:
        # for num in words:
            for one in dic1[words]:
                encode.append(one)
    return encode

def myencode2(str):
    encode=[]
    for words in jieba.lcut(word):
        # for one in words:
...
for step in list_steps:
    ...
    for i in range(3):
        list2=[]
        list2=myencode2(step[i])
        for num in list2:
            list.append(num)
    if len(list)>50:
        for i in range(len(list)-1,50,-1):
            list.pop(i)
    else:
        for i in range(1,50-len(list)+1):
            list.append(0)
    list=np.array(list)
