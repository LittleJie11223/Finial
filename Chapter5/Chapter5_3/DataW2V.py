...
def mydictionary(list1, list2,mysize):
    listW=[]
    for steps in list2:
        listW11=[]
        for step in steps:
    ...
    for ingredient in list1:
        ...
        for word in ingredient:
            listW22.append(word)
        listW.append(listW22)
    model = Word2Vec(listW, size=mysize, window=5, min_count=1, workers=4)

    dic = {}
    for recipe in listW:
        for word in recipe:
            if mysize==1:
                dic[word]=model[word][0]
            else:
                dic[word]=model[word]
    return dic
...
def myencode(dic,str):
    encode = []
    for words in str:
        if type(dic[words])==np.float32:
            encode.append((dic[words]))
        else:
            for num in dic[words]:
                encode.append(num)
    return encode

def mydecode(dic,str,mysize):
    decode = ''
    for i in range(0,len(str)-mysize+1,mysize):
        list=np.array(str[i:i+mysize])
        error=100000
        mykey=''
        for key,value in dic.items():
            a=np.linalg.norm(np.array(value-list), ord=2, axis=0, keepdims=False)
            if error>a:
                error=a
                mykey=key
        if mykey!='':
...
def find_inputsize(list_ingredients,mysize):
    row_input=0
    column_input=1
...
def find_outputsize(list_steps,mysize):
    row_output=0
    column_output=3
    for step in list_steps:
        for i in range(3):
...
def getData(mysize):
    ...
    dic = mydictionary(list_ingredients, list_steps,mysize)
    # build the input matrix and output matrix
    # Firstly, make sure the size of input and output
    row_input, column_input = find_inputsize(list_ingredients,mysize)
    row_output, column_output = find_outputsize(list_steps,mysize)
    # build the input matrix
